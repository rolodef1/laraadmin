...
class CodeGenerator
{
	...
	**/
    ...
    public static function createApiController($config, $comm = null) {
        ...
        $md = str_replace("__db_table_name__", $config->dbTableName, $md);
        $md = str_replace("__singular_var__", $config->singularVar, $md);
        if (!file_exists(base_path('app/Http/Controllers/apiLA'))) {
    ...
    public static function appendApiRoutes($config, $comm = null) {

        $templateDirectory = __DIR__.'/stubs';

        LAHelper::log("info", "Appending api routes...", $comm);
        if(\Dwij\Laraadmin\Helpers\LAHelper::laravel_ver() == 5.3) {
            $routesFile = base_path('routes/api_routes.php');
        } else {
            $routesFile = app_path('Http/api_routes.php');
        }
        ...
        $md = file_get_contents($templateDirectory."/apiroutes.stub");
